...
const char rc[] = "123A456B789C*0#D";
...
{
	for (int i = 0; i < 4; i++) {
		filt[i] = new_goertzel(DEFAULT_FRAMESIZE, row[i], 8000);
		assert(filt[i]);
	}
	for (int i = 0; i < 4; i++) {
		filt[i+4] = new_goertzel(DEFAULT_FRAMESIZE, col[i], 8000);
		assert(filt[i+4]);
	}
...
{
	double phaseincr = 2.0 * M_PI * low / SR;
	double phase = 0;
	int length = ch_duration * SR / 1000;
	//printf("dtmf_custom_freq: low %d, high %d, length %d\n", low, high, length);
	if (length > 16384) length = 16384;

	for (int i = 0; i < length; i++) {
		data[i] = 0.5 * sin(phase);
		phase += phaseincr;
	}
	...
	for(int i=0;i<length;i++){
		...
		//printf("dtmf_custom_freq: %d, %f, %d\n", i, data[i], outbuf[i]);
	}

	return length;
}

static int dtmf_silence(short *outbuf)
{
...
static int dtmf_two_tone(int ch, short *outbuf, int window)
{
	ch = toupper(ch);
	if(ch=='E') ch = '*';
	if(ch=='F') ch = '#';
	...
	int r = pos / 4;
	int c = pos % 4;
	//printf("dtmf_two_tone: %c\n", ch);
	return dtmf_custom_freq(row[r], col[c], outbuf, window);
}
...
int dtmf_send(char *ch, short *outbuf, int window){
	...
	for(int i = 0; i < len; i++) {
		int c = ch[i];
		if (c == ' ' || c == ',' || c == '-') {
			out_len += dtmf_silence(&outbuf[out_len]);
		}else {
			out_len += dtmf_two_tone(c, &outbuf[out_len], window);
		}
		out_len += dtmf_silence(&outbuf[out_len]);
	}
	return out_len;
...
// reset row freq filters
	for (int i = 0; i < 4; i++) goertzel_reset(filt[i], DEFAULT_FRAMESIZE, row[i], SR);
// reset col freq filters
	for (int i = 0; i < 4; i++) goertzel_reset(filt[i+4], DEFAULT_FRAMESIZE, col[i], SR);
...
int dtmf_receive(const short* buf, int len, char * outbuf)
{
	...
	while (1) {
		...
		x = decode(); 
		printf("decode: x='%c', last='%c', silence_time=%d\n", x, last, silence_time);
		if (x == ' ') {
			silence_time++;
